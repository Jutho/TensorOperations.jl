...
function tensorcopy(IC::Tuple, A, IA::Tuple, conjA::Symbol=:N, α::Number=_one)
    pC = add_indices(IA, IC)
    return tensorcopy(pC, A, conjA, α)
end
# default `IC`
function tensorcopy(A, IA, conjA::Symbol=:N, α::Number=_one)
    return tensorcopy(tuple(IA...), A, tuple(IA...), conjA, α)
end
# implement for iterables
function tensorcopy(IC, A, IA, conjA::Symbol=:N, α::Number=_one)
    return tensorcopy(tuple(IC...), A, tuple(IA...), conjA, α)
end
# expert mode
function tensorcopy(pC::Index2Tuple, A, conjA::Symbol=:N, α::Number=_one,
                    backend::Backend...)
...
"""
function tensorcopy!(C, pC::Index2Tuple, A, conjA::Symbol=:N, α::Number=_one,
                     backend::Backend...)
...
function tensoradd(IC::Tuple, A, IA::Tuple, conjA::Symbol, B, IB::Tuple,
                   conjB::Symbol, α::Number=_one, β::Number=_one)
...
function tensoradd(IC, A, IA, B, IB, α::Number=_one, β::Number=_one)
    return tensoradd(tuple(IC...), A, tuple(IA...), :N, B, tuple(IB...), :N, α, β)
end
# default `IC`, `conjA` and `conjB`
function tensoradd(A, IA, B, IB, α::Number=_one, β::Number=_one)
...
function tensoradd(A, pA::Index2Tuple, conjA::Symbol,
                   ...
                   α::Number=_one, β::Number=_one, backend::Backend...)
    ...
    return tensoradd!(C, pB, B, conjB, β, _one, backend...)
end
...
function tensortrace(IC::Tuple, A, IA::Tuple, conjA::Symbol=:N, α::Number=_one)
    pC, cindA1, cindA2 = trace_indices(IA, IC)
    return tensortrace(pC, A, (cindA1, cindA2), conjA, α)
end
# default `IC`
function tensortrace(A, IA, conjA::Symbol, α::Number=_one)
    return tensortrace(unique2(tuple(IA...)), A, tuple(IA...), conjA, α)
end
# default `conjA`
function tensortrace(IC, A, IA, α::Number=_one)
    return tensortrace(tuple(IC...), A, tuple(IA...), :N, α)
end
# default `IC` and `conjA`
function tensortrace(A, IA, α::Number=_one)
    return tensortrace(unique2(tuple(IA...)), A, tuple(IA...), :N, α)
end
# iterables
function tensortrace(IC, A, IA, conjA::Symbol, α::Number=_one)
    return tensortrace(tuple(IC...), A, tuple(IA...), conjA, α)
end
# expert mode
function tensortrace(pC::Index2Tuple, A, pA::Index2Tuple, conjA::Symbol, α::Number=_one,
                     backend::Backend...)
    TC = promote_contract(scalartype(A), scalartype(α))
    C = tensoralloc_add(TC, pC, A, conjA)
    return tensortrace!(C, pC, A, pA, conjA, α, _zero)
end
...
# default `IC`
function tensorcontract(A, IA, conjA, B, IB, conjB, α::Number=_one)
    return tensorcontract(symdiff(tuple(IA...), tuple(IB...)), A, tuple(IA...), conjA, B,
                          tuple(IB...), conjB, α)
end
# default `conjA` and `conjB`
function tensorcontract(IC, A, IA, B, IB, α::Number=_one)
    return tensorcontract(tuple(IC...), A, tuple(IA...), :N, B, tuple(IB...), :N, α)
end
# default `IC`, `conjA` and `conjB`
function tensorcontract(A, IA, B, IB, α::Number=_one)
    return tensorcontract(symdiff(tuple(IA...), tuple(IB...)), A, tuple(IA...), :N, B,
                          tuple(IB...), :N, α)
end
# iterables
function tensorcontract(IC, A, IA, conjA::Symbol, B, IB, conjB::Symbol, α::Number=_one)
...
function tensorcontract(pC::Index2Tuple, A, pA::Index2Tuple, conjA::Symbol, B,
                        pB::Index2Tuple, conjB::Symbol, α::Number=_one, backend::Backend...)
    TC = promote_contract(scalartype(A), scalartype(B), scalartype(α))
    C = tensoralloc_contract(TC, pC, A, pA, conjA, B, pB, conjB)
    return tensorcontract!(C, pC, A, pA, conjA, B, pB, conjB, α, _zero, backend...)
end
...
# default `IC`
function tensorproduct(A, IA, conjA::Symbol, B, IB, conjB::Symbol, α::Number=_one)
    return tensorproduct(vcat(tuple(IA...), tuple(IB...)), A, tuple(IA...), conjA, B,
                         tuple(IB...), conjB, α)
end
# default `conjA` and `conjB`
function tensorproduct(IC, A, IA, B, IB, α::Number=_one)
    return tensorproduct(tuple(IC...), A, tuple(IA...), :N, B, tuple(IB...), :N, α)
end
# default `IC`, `conjA` and `conjB`
function tensorproduct(A, IA, B, IB, α::Number=_one)
    return tensorproduct(vcat(tuple(IA...), tuple(IB...)), A, tuple(IA...), :N, B,
                         tuple(IB...), :N, α)
end
# iterables
function tensorproduct(IC, A, IA, conjA::Symbol, B, IB, conjB::Symbol, α::Number=_one)
...
function tensorproduct!(C, pC::Index2Tuple,
                        ...
                        B, pB::Index2Tuple, conjB::Symbol,
                        α::Number=_one, β::Number=_zero, backend::Backend...)
    numin(pA) == 0 && numout(pB) == 0 ||
        throw(IndexError("not a valid tensor product"))
