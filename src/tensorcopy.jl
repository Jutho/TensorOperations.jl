...
@ngenerate N typeof(C) function tensorcopy!{TA,TC,N}(A::StridedArray{TA,N},labelsA,C::StridedArray{TC,N},labelsC)
...
# @ngenerate N typeof(C) [1,2,3,4,5,6,7,8] function permutedims2!{TA,TC,N}(C::StridedArray{TC,N},A::StridedArray{TA,N},perm,basesize::Int=1024)
...
#     end
#
#     if length(C)<=4*basesize
#         @stridedloops(N, i, dims, indA, startA, stridesA, indC, startC, stridesC, @inbounds Clinear[indC]=Alinear[indA])
#     else
#         @nexprs N d->(minstrides_{d} = min(stridesA_{d},stridesC_{d}))
#
#         # build recursive stack
#         depth=iceil(log2(length(C)/basesize))+2 # 2 levels safety margin
#         level=1 # level of recursion
...
#             bstartC=stackbstartC[level]
#
#             if blength<=basesize || level==depth # base case
#                 @stridedloops(N, i, bdims, indA, bstartA, stridesA, indC, bstartC, stridesC, @inbounds Clinear[indC]=Alinear[indA])
#                 level-=1
#             elseif step==0
#                 # find which dimension to divide
