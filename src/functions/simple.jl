...
The keyword argument `method` can take the values `:BLAS` or `:native`. The first
option creates temporary copies of `A`, `B` and the resulting tensor where the indices
are permuted such that the contractions become equivalent to a single matrix multiplication,
which is then handled by BLAS. This is often the fastest approach and therefore the default
value, but it does require sufficient memory and there is some overhead in allocating
new memory (e.g. when doing this many times in a loop). In case `method` is set to
`:native`, a native Julia function is called that performs the contraction without
creating tempories, with special attention to cache-friendliness for maximal efficiency.
This can be faster for small tensors or when the contraction pattern is non-generic,
