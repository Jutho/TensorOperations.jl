...
@testset "Index Notation $withblas BLAS and $withcache cache" begin
    ...
    operationbackend = StridedBackend(withblas)
    allocationbackend = JuliaAllocator()
    for f in (contractbackend!, addbackend!, tracebackend!)
        f(operationbackend)
    end
    for g in (allocatebackend!, allocatetempbackend!)
        g(allocationbackend)
    end
    ...
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64, BigFloat)
        ...
        @tensor HrA12′′′[a, s1, s2, c] := rhoL[a, a'] * H[s1, s2, t1, t2] * A2[b, t2, c'] *
                                          rhoR[c', c] * A1[a', t1, b] order = (a', b, c',
                                                                               t1, t2)# should be contracted in exactly same order
    ...
    @testset "tensoropt" begin
        ...
        B = randn(5, 5, 5)
        C = randn(5, 5, 5)
        @tensoropt (a => χ, b => χ^2, c => 2 * χ, d => χ, e => 5, f => 2 * χ) D1[a, b, c, d] := A[a,
                                                                                                  e,
                                                                                                  c,
                                                                                                  f] *
                                                                                                B[g,
                                                                                                ...
                                                                                                C[g,
                                                                                                  f,
                                                                                                  b]
        @tensoropt (a=χ, b=χ^2, c=2 * χ, d=χ, e=5, f=2 * χ) D2[a, b, c, d] := A[a, e, c,
                                                                                f] *
                                                                              B[g, d, e] *
                                                                              C[g, f, b]
        @tensoropt ((a, d) => χ, b => χ^2, (c, f) => 2 * χ, e => 5) D3[a, b, c, d] := A[a,
                                                                                        e,
                                                                                        c,
                                                                                        f] *
                                                                                      B[g,
                                                                                      ...
                                                                                      C[g,
                                                                                        f,
                                                                                        b]
        @tensoropt ((a, d)=χ, b=χ^2, (c, f)=2 * χ, e=5) D4[a, b, c, d] := A[a, e, c, f] *
                                                                          B[g, d, e] *
                                                                          C[g, f, b]
        @test D1 == D2 == D3 == D4
