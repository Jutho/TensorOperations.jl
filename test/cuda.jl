...
@testset "more complicated expressions" verbose=true begin
    ...
    @tensor D2[d, f, h] := CuArray(A)[c, a, f, a, e, b, b, g] * CuArray(B)[c, h, g, e, d] + 0.5 * CuArray(C)[d, h, f]
    @test collect(D2) ≈ D1
    ...
    @testset "readme example" begin
        ...
        @tensor begin
            D2[a, b, c] = CuArray(A)[a, e, f, c, f, g] * CuArray(B)[g, b, e] + α * CuArray(C)[c, a, b]
            E2[a, b, c] := CuArray(A)[a, e, f, c, f, g] * CuArray(B)[g, b, e] + α * CuArray(C)[c, a, b]
        end
        @test collect(D2) ≈ D
        @test collect(E2) ≈ E
    end
    
    @testset "tensor network examples ($T)" for T in (Float32, Float64, ComplexF32, ComplexF64)
        D1, D2, D3 = 30, 40, 20
        d1, d2 = 2, 3
        ...
        @tensor begin
            HrA12[a, s1, s2, c] := ρₗ[a, a'] * A1[a', t1, b] * A2[b, t2, c'] * ρᵣ[c', c] * H[s1, s2, t1, t2]
        end
        @tensor begin
            HrA12′[a, s1, s2, c] := CuArray(ρₗ)[a, a'] * CuArray(A1)[a', t1, b] * CuArray(A2)[b, t2, c'] * CuArray(ρᵣ)[c', c] * CuArray(H)[s1, s2, t1, t2]
        end
        @test collect(HrA12′) ≈ HrA12
        
        @tensor begin
            E1 = ρₗ[a', a] * A1[a, s, b] * A2[b, s', c] * ρᵣ[c, c'] * H[t, t', s, s'] * conj(A1[a', t, b']) * conj(A2[b', t', c'])
            E2 = CuArray(ρₗ)[a', a] * CuArray(A1)[a, s, b] * CuArray(A2)[b, s', c] * CuArray(ρᵣ)[c, c'] * CuArray(H)[t, t', s, s'] * conj(CuArray(A1)[a', t, b']) * conj(CuArray(A2)[b', t', c'])
        end
        @test E2 ≈ E1
    end
end
