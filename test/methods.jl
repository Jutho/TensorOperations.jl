...
mixed_scalars = TensorOperations.backend() in (:Strided, :StridedBLAS)
...
for T in (Float64, ComplexF64)
    @testset "$T" verbose = true begin
        ...
        @testset "tensorcopy!" begin
            Abig = randn(T, (30, 30, 30, 30))
            A = view(Abig, 1 .+ 3 * (0:9), 2 .+ 2 * (0:6), 5 .+ 4 * (0:6), 4 .+ 3 * (0:8))
            p = (3, 1, 4, 2)
            Cbig = zeros(T, (50, 50, 50, 50))
        ...
        @testset "tensorcontract!" begin
            Abig = rand(T, (30, 30, 30, 30))
            A = view(Abig, 1 .+ 3 * (0:8), 2 .+ 2 * (0:14), 5 .+ 4 * (0:6), 7 .+ 2 * (0:8))
            Bbig = rand(T, (50, 50, 50))
            B = view(Bbig, 3 .+ 5 * (0:6), 7 .+ 2 * (0:7), 13 .+ (0:14))
            Cbig = rand(T, (40, 40, 40))
